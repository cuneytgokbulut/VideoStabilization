...
public class ColorProcessor extends ImageProcessor {
	...
	*/
	...
	public void fromTop(int margin) {
		if (margin==0) return;
		else if (margin<0) {
			margin = -margin;
			for (int j = 0; j < roiWidth; j++) {
				for (int i = roiHeight - 2; i > margin - 1; i--) {
					pixels[j + i * roiWidth] = pixels[j + (i-margin)*roiWidth];
				}
				for (int x = 0; x < margin; x++) {
					pixels[j + x * roiWidth] = bgColor;
				}
			}
		}
		else {
			for (int j = 0; j < roiWidth; j++) {
				for (int i = 0; i < roiHeight - margin - 1; i++) {
					pixels[j + i * roiWidth] = pixels[j + (i+margin)*roiWidth];
				}
				for (int x = roiHeight-margin; x < roiHeight; x++) {
					pixels[j + x * roiWidth] = bgColor;
				}
			}
		}
	...
	public void toLeft(int margin) {
		if (margin==0) return;
		else if (margin>0) {
			for (int i = 0; i < roiHeight; i++) {
				for (int j = roiWidth - 2; j > margin - 1; j--) {
					pixels[j + i * roiWidth] = pixels[j + i * roiWidth - margin];
				}
				for (int x = 0; x < margin; x++) {
					pixels[x + i * roiWidth] = bgColor;
				}
			}
		}
		else {
			for (int i = 0; i < roiHeight; i++) {
				for (int j=0; j< roiWidth+margin;j++) {
					pixels[j + i * roiWidth] = pixels[j + i * roiWidth - margin];
				}
				for (int x = roiWidth-1; x>roiWidth+margin; x--) {
					pixels[x + i * roiWidth] = bgColor;
				}
			}
		}
	...
	public void applyParameters(double angle,int top, int left, double scale) {
		int[] pixels2 = (int[])getPixelsCopy();
		...
		for (int y=0; y<height; y++) {
			...
			for (int x=0; x<=width - 1; x++) {
				...
				if ((xs>=-0.01) && (xs<dwidth) && (ys>=-0.01) && (ys<dheight)) {
					...
					int indis = width * iys + ixs - left + width * top;
					if (pixels2.length>indis && indis>=0) {
						pixels[index++] = getScaledIndis(indis,scale,pixels2);
					}
	...
	private int getScaledIndis(int indis, double sc, int[] pixels2) {
		...
		ys = (y-yCenter)/sc + yCenter;
		if (ys<0.0) ys = 0.0;
		if (ys>=ylimit) ys = ylimit2;
		index2 = width*(int)ys;

		xs = (x-xCenter)/sc + xCenter;
		xsi = (int)xs;
// if (pixels2.length<=index2+xsi) return bgColor;
		return pixels2[index2+xsi];
	}
